#!/usr/bin/env python3
"""Train-slice evaluation for QURA checkpoint artifacts.

This file intentionally does not import from eval/test.  It mirrors the
standard evaluator's checkpoint contract, but scores a fixed stratified half of
the CIFAR test set so future agents can tune against this visible slice.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate QURA artifacts on the visible train slice")
    ...
    parser.add_argument("--methods_from_scores", default=None)
    parser.add_argument("--output", default="/home/user/scoring/scores_train.json")
